...
export class OTPService {
  ...
  /**
   * Generates, stores, and sends a password reset OTP to the given email.
   * Enforces a 60-second cooldown from the previous active password reset OTP.
   * Invalidate any old password reset OTP before dispatching the new one.
   */
  async generateAndSendPasswordResetOTP(email: string): Promise<{ success: boolean; cooldownSeconds: number }> {
    const normalizedEmail = email.toLowerCase().trim();
    ...
    // Check for active password reset OTP and enforce 60-second resend cooldown
    const existingOtp = await this.otpRepo.findLatestByEmail(normalizedEmail, "password_reset");
    if (existingOtp && existingOtp.createdAt) {
      const elapsed = Date.now() - new Date(existingOtp.createdAt).getTime();
      if (elapsed < this.RESEND_COOLDOWN_MS) {
        const remainingSeconds = Math.ceil((this.RESEND_COOLDOWN_MS - elapsed) / 1000);
        throw new BadRequestError(
          `Please wait ${remainingSeconds} seconds before requesting a new password reset code.`,
        );
      }
    }

    // Invalidate/delete previous password reset OTPs for this email to prevent replay
    await this.otpRepo.deleteByEmail(normalizedEmail, "password_reset");
    ...
    // Dispatch OTP via Email Service
    const emailResult = this.emailService.sendPasswordResetOTP
      ? await this.emailService.sendPasswordResetOTP(normalizedEmail, plainOtp)
      : await this.emailService.sendEmail({
          to: normalizedEmail,
          subject: "Reset Your Password - HelthGate Hospital Management",
          html: `<p>Your password reset code is: <strong>${plainOtp}</strong></p>`,
          text: `Your password reset code is: ${plainOtp}`,
        });

    if (!emailResult.success) {
      console.warn(`[OTPService] Outbound password reset email warning for ${normalizedEmail}: ${emailResult.error}`);
    }
  ...
  /**
   * Verifies the provided password reset OTP for the given email.
   * Enforces expiration, attempt limits (max 5), and marks OTP as used upon success.
   */
  async verifyPasswordResetOTP(email: string, candidateOtp: string): Promise<boolean> {
    const normalizedEmail = email.toLowerCase().trim();
    ...
    const otpRecord = await this.otpRepo.findLatestByEmail(normalizedEmail, "password_reset");
    if (!otpRecord) {
      throw new BadRequestError("No active password reset code found for this email. Please request a new code.");
    }
